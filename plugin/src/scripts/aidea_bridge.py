#!/usr/bin/env python3
"""
aidea_bridge.py - bridge between Iris Zotero plugin and pdf2zh_next CLI
...
def _http_post_json_with_retry(
    ...
    while True:
        try:
            ...
            print(
                f"[Iris] transient upstream error from {url}; retrying "
...
class OAuthCompatProxyServer:
    ...
    def _forward_gemini(self, payload):
        ...
        if not project_id:
            raise RuntimeError(
                "Missing Google project ID for Gemini OAuth. Re-login in Iris settings."
            )
